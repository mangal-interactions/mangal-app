<template>
  <div>
    <v-tabs slot="extension" centered color="white" slider-color="teal">
      <v-tab ripple>
        Network
      </v-tab>
      <v-tab-item>
            ...
            <v-layout justify-center>
              <v-flex x12 pa-2 class="text-md-center">
                <div
                  ...
                  style="font-family:Cardo!important;"
                >
                  {{ selectNet.description }}
                </div>
                <div>
                  <v-icon small color="white">fas fa-info-circle</v-icon> Mangal
                  network ID #{{ selectNet.id }}, measurement date:
                  {{ selectNet.date | moment }}
                </div>
                <v-chip
                  small
                  v-if="selectNet.public"
                  color="teal"
                  text-color="white"
                  >Public</v-chip
                >
                <v-chip small v-else color="orange" text-color="white"
                  >Private</v-chip
                >
                <v-chip
                  small
                  v-if="!selectNet.all_interactions"
                  color="red"
                  text-color="white"
                  >Unrecorded absences</v-chip
                >
                <v-chip small v-else color="green" text-color="white"
                  >Recorded absences</v-chip
                >
              </v-flex>
            </v-layout>
          </v-card>
              ...
              <v-card-text>
                ...
                <span v-else>
                  <p
                    ...
                    style="padding-top:10rem"
                  >
                    Network not rendering (more than 200 nodes)
                  </p>
                </span>
              </v-card-text>
            </v-card>
...
export default {
  ...
  watch: {
    selectNet: function(newNet) {
      ...
      const datasetId =
        newNet.datasetId !== undefined ? newNet.datasetId : newNet.dataset_id;

      this.$store.dispatch("loadDataset", datasetId).then(() => {
        // Update ref
        this.$store.dispatch("loadRef", this.$store.state.dataset.refId);
      });
    }
  },
  filters: {
    ...
    updateNetwork(idNet) {
      ...
      this.$store
        ...
        .then(taxaIds => {
          ...
          this.$store
            .dispatch("loadInteractions", taxaIds)
            .then(() => {
            ...
            .then(() => {
              ...
              this.con = (this.nEdges / this.getTaxons().length ** 2).toFixed(
                2
              );
            })
            .catch(err => {
  ...
  mounted() {
    ...
    this.$store
      .dispatch("loadDataset", this.getNetCollection()[0].dataset_id)
      .then(() => {
        // Update ref
        this.$store.dispatch("loadRef", this.$store.state.dataset.refId);
      });
